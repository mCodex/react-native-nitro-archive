...
export function uriDestination(
  ...
  return Object.freeze({ kind: 'uri', uri, iosSecurityScope: options.iosSecurityScope ?? 'auto' })
}

export function directoryDestination(path: string): DirectoryPathDestination {
  if (!path.startsWith('/')) {
    throw new TypeError('directoryDestination(path) requires an absolute path.')
  }
...
export function directoryUriDestination(
  ...
  if (!uri.includes('://')) {
    throw new TypeError('directoryUriDestination(uri) requires an absolute URI.')
  }
  return Object.freeze({ kind: 'directory-uri', uri, iosSecurityScope: options.iosSecurityScope ?? 'auto' })
}
