...
import { mapEntry, mapEntryPage, mapExtractionOptions, mapValidationOptions } from './nativeAdapter'
...
export class ArchiveReaderWrapper implements ArchiveReader {
  ...
  get compressedSize() {
    return this.native.compressedSize != null ? BigInt(this.native.compressedSize) : undefined
  }

  get totalUncompressedSize() {
    return this.native.totalUncompressedSize != null ? BigInt(this.native.totalUncompressedSize) : undefined
  }
  ...
  async listEntries(options?: import('../types/archive').ListArchiveEntriesOptions) {
    return mapEntryPage(
      await this.native.listEntries(
        options?.offset ?? 0,
        options?.limit ?? 250,
        options?.prefix ?? undefined,
        options?.kinds != null ? ([...options.kinds] as readonly string[]) : undefined,
      ),
    )
  }
  ...
  async readEntry(path: string, options: { maxBytes: bigint; password?: string; verifyChecksum?: boolean }) {
    return this.native.readEntry(path, options.maxBytes, options.password, options.verifyChecksum)
  }
