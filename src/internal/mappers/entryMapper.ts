...
import type { ArchiveEntry, ArchiveEntryPage } from "../../types/archive";
...
export function mapEntry(native: NativeArchiveEntry): ArchiveEntry {
	return {
		...
		compressionMethod:
			native.compressionMethod as ArchiveEntry["compressionMethod"],
		modifiedAt:
			native.modifiedAt != null ? new Date(native.modifiedAt) : undefined,
	...
	switch (entry.kind) {
		...
		case "buffer":
			return {
				...
				archivePath: entry.archivePath,
				modifiedAt:
					entry.modifiedAt != null ? entry.modifiedAt.getTime() : undefined,
				compressionMethod: entry.compressionMethod,
			};
		default:
			throw new ArchiveError(
				"E_INVALID_ARGUMENT",
				`Unknown entry kind: ${(entry as ArchiveEntryInput).kind}`,
			);
	}
}
