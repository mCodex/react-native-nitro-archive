...
export function directoryEntry(
  sourcePath: string,
  archivePath: string,
  options: Omit<DirectoryArchiveEntryInput, 'kind' | 'sourcePath' | 'archivePath'> = {},
): DirectoryArchiveEntryInput {
  if (!sourcePath.startsWith('/')) {
    throw new TypeError('directoryEntry(sourcePath) requires an absolute path.')
  }
...
export function bufferEntry(
  data: ArrayBuffer,
  archivePath: string,
  options: Pick<BufferArchiveEntryInput, 'modifiedAt' | 'compressionMethod'> = {},
): BufferArchiveEntryInput {
