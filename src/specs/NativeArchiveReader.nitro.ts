...
export interface NativeArchiveReader
  ...
  listEntries(offset: number, limit: number, prefix?: string, kinds?: readonly string[]): Promise<NativeEntryPage>
  getEntry(path: string): Promise<NativeArchiveEntry | undefined>
  readEntry(path: string, maxBytes: UInt64, password?: string, verifyChecksum?: boolean): Promise<ArrayBuffer>
